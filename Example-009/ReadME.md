...
<li>the implementation for the <i>custom</i> functions are interest are stored in the <i>'myfunctions.cpp'</i> C++ source file (in the <i>'src'</i> folder)</li></ul><p>Several methods for its building are provided, and take into the account the source code organization (described above).<p><hr><p>

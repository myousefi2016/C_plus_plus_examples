/**********************************************************************************************
 * Examples in the C++ language (the test 'Example-018')
 *
 * GitHub repository: http://github.com/davidcanino/C_plus_plus_examples
 *
 * Created by David Canino (canino.david@gmail.com), December 2016
 *
 * main.cpp - the source file, implementing the main function for the test 'Example-018'.
 **********************************************************************************************/
...
/// The main function for the <i>Example-018</i> unit test.
int main(void)
{
	/* This is the example 'Example-018', where the properties of the 'int' type are analyzed. */
    cout<<endl<<"\tThis is the example 'Example-018' in the C++ language"<<endl<<endl;
    cout.flush();
	cout<<"\tThe C++ built-in 'int' type:"<<endl<<endl;
	...
	if(std::numeric_limits<int>::has_infinity) cout<<"\t-) represents the 'infinity' value as "<<std::numeric_limits<int>::infinity()<<endl;
	...
	if(std::numeric_limits<int>::has_denorm) cout<<"\t-) represents the subnorm values"<<endl;
	...
	{	
		case round_toward_zero:
			...
			cout<<"\t-) exploits the 'round toward zero' policy for rounding not completely 'int' values"<<endl;
			break;
			
		case round_to_nearest:
		
			cout<<"\t-) exploits the 'round to nearest' policy for rounding not completely 'int' values"<<endl;
			break;
			
		case round_toward_infinity:
		
			cout<<"\t-) exploits the 'round to the positive infinity' policy for rounding not completely 'int' values"<<endl;
			break;
			
		case round_toward_neg_infinity:
		
			cout<<"\t-) exploits the 'round to the negative infinity' policy for rounding not completely 'int' values"<<endl;
			break;

		default:
		
			cout<<"\t-) does not exploit a policy for rounding not completely 'int' values"<<endl;
			cout.flush();
			break;
	}
	
	/* Now, we finalize our analysis! */
	if(std::numeric_limits<int>::is_iec559) cout<<"\t-) fulfills the requirements of IEC 559 (IEEE 754) standard"<<endl;
	...
	cout<<"\t-) has "<<(int)std::numeric_limits<int>::min()<<" as the minimum finite value, representable as a 'int' value"<<endl;
	cout<<"\t-) has "<<(int)std::numeric_limits<int>::min()<<" as the lowest finite value, representable as a 'int' value"<<endl;
	cout<<"\t-) has "<<(int)std::numeric_limits<int>::max()<<" as the maximum finite value, representable as a 'int' value"<<endl;
	cout<<"\t-) has "<<(int)std::numeric_limits<int>::epsilon()<<" as the machine epsilon, that is, the difference between 1.0 and the next value, representable as a 'int' value"<<endl;
	cout<<"\t-) has "<<(int)std::numeric_limits<int>::round_error()<<" as the maximum rounding error"<<endl;
	cout<<"\t-) has "<<(int)std::numeric_limits<int>::denorm_min()<<" as the minimum positive subnormal value"<<endl<<endl;
	return EXIT_SUCCESS;
}

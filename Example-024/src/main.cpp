/**************************************************************************************************
 * Examples in the C++ language (the 'Example-024' unit test).
 *
 * GitHub repository: http://github.com/davidcanino/C_plus_plus_examples
 *
 * Created by David Canino (canino.david@gmail.com), March 2017
 *
 * main.cpp - the source file, implementing the main function for the 'Example-024' unit test.
 **************************************************************************************************/
...
/// The main function for the <i>'Example-024'</i> unit test.
int main(void)
{
	/* This is the 'Example-024' unit test, where the properties of the 'UTF-16 character (char16_t)' C++ built-in type are analyzed. */
    cout<<endl<<"\tThis is the 'Example-024' unit test in the C++ language."<<endl<<endl;
    cout.flush();
    
    /* TASK #1 - retrieving the properties of the 'UTF-16 character (char16_t)' C++ built-in type. */
	cout<<"\tThe 'UTF-16 character (char16_t)' C++ built-in type:"<<endl<<endl;	
	...
	if(std::numeric_limits<char16_t>::has_infinity) cout<<"\t-) represents the 'infinity' value as "<<std::numeric_limits<char16_t>::infinity()<<";"<<endl;
	...
	if(std::numeric_limits<char16_t>::has_denorm) cout<<"\t-) represents the subnorm values;"<<endl;
	...
	{	
		case round_toward_zero:
			...
			cout<<"\t-) exploits the 'round toward zero' policy for rounding not completely 'char16_t' values;"<<endl;
			break;
			
		case round_to_nearest:
		
			cout<<"\t-) exploits the 'round to nearest' policy for rounding not completely 'char16_t' values;"<<endl;
			break;
			
		case round_toward_infinity:
		
			cout<<"\t-) exploits the 'round to the positive infinity' policy for rounding not completely 'char16_t' values;"<<endl;
			break;
			
		case round_toward_neg_infinity:
		
			cout<<"\t-) exploits the 'round to the negative infinity' policy for rounding not completely 'char16_t' values;"<<endl;
			break;

		default:
		
			cout<<"\t-) does not exploit a policy for rounding not completely 'char16_t' values;"<<endl;
			cout.flush();
			break;
	}
	
	/* TASK #2 - finalizing our analysis! */
	if(std::numeric_limits<char16_t>::is_iec559) cout<<"\t-) fulfills the requirements of IEC 559 (IEEE 754) standard;"<<endl;
	...
	cout<<"\t-) has "<<(char16_t)std::numeric_limits<char16_t>::epsilon()<<" as the machine epsilon, that is, the difference between 1.0 and the next value, representable as a 'char16_t' value;"<<endl;
	cout<<"\t-) has "<<(char16_t)std::numeric_limits<char16_t>::round_error()<<" as the maximum rounding error;"<<endl;
	cout<<"\t-) has "<<(char16_t)std::numeric_limits<char16_t>::denorm_min()<<" as the minimum positive subnormal value."<<endl<<endl;
	return EXIT_SUCCESS;
}

...
This is a basic example about the use of the C++ programming language. Basically, this program exploits the infrastructure, introduced for the first time by the C++11 standard (thus the '<i>numeric_limits</i>' template class), for analyzing several properties of the C++ <i>'unsigned long long integer (unsigned long long)'</i> built-in type.<p>In particular, this test consists of the <i>'Example-029'</i> executable file, starting uniquely from the <i>'main.cpp'</i> C++ source file (in the <i>'src'</i> folder) without using other dependencies.<p>Several methods for its building are provided.<p><table border=1 width=100%><tr><td><p><b>VERY IMPORTANT:</b><p>In other words, this test can be used if your C++ compiler is a C++11-compliant compiler. The most recent versions of the compilers in the current literature are C++11-compliant.<p></td></tr></table><p><hr><p>

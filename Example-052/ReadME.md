...
This basic example about the use of the C++ programming language validates explicitly the use of the <i>'do-while'</i> and the <i>'switch'</i> constructs (including also other constructs), even if they have been exploited in other previous tests. These constructs are exploited for defining a very dummy application, which computes the total cost for purchasing several memberships fares. The shipping rate depends on their validity and on the customer age.<p>In particular, this test consists of the <i>'Example-052'</i> executable file, starting uniquely from the <i>'main.cpp'</i> C++ source file (in the <i>'src'</i> folder) without using other dependencies.<p>Several methods for building this test are provided.<p><hr><p>

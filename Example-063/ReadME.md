...
This basic test about the use of the C++ programming language validates the member functions, offered by the <i>'std::map'</i> template class (defined in the <i>'< map >'</i> header file). This latter describes an associative container, which stores several pairs, that are formed by a <i>'key'</i> value <code><i>'k'</i></code> and a <i>'mapped' value</i>, corresponding to <code><i>'k'</i></code>. These associations are stored by following the lexicographic order of their keys. In this case, it is not possible to store more than an association, which corresponds to any <i>'key'</i> value <code><i>'k'</i></code>. In this test, also the new capabilities of the <i>'std::map'</i> template class, introduced recently in the C++11 standard, are validated.<p>

...
This basic example about the use of the C++ programming language validates the use of the member functions, offered by the <i>'std::list'</i> template class (defined in the <i>'std::list'</i> header file). This latter describes a generic container, containing several template values, which represents a <i>dynamic double-linked list</i>. In particular, also the new capabilities, introduced recently in the C++11 standard are tested.<p>In particular, this test consists of the <i>'Example-072'</i> executable file, starting uniquely from the <i>'main.cpp'</i> C++ source file (in the <i>'src'</i> folder) without using other dependencies.<p>Several methods for building this test are provided.<p><table border=1 width=100%><tr><td><p><b>VERY IMPORTANT:</b><p>It is clear that this test can be built and run ONLY if your C++ compiler is a C++11-compliant compiler. The most recent versions of the compilers in the current literature are C++11-compliant.<p></td></tr></table><p><hr><p>

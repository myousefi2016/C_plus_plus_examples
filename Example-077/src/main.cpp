...
int main(void)
{
    ...
    cout<<"\tCreating an empty multiset 's0' of 'ulong' values ... ";
 	...
 	multiset<ulong> s0;
 	cout<<"ok"<<endl;
 	cout<<"\t#values in the multiset 's0': "<<s0.size()<<endl;
    if(s0.empty()==true) cout<<"\tThe multiset 's0' is empty"<<endl<<endl;
 	...
 	ulong ls[]= {50,30,20,10,40,10,40,10,10};
  	...
  	cout<<"ok"<<endl;
  	cout<<"\t#values in the multiset 's1': "<<s1.size()<<endl;
    if(s1.empty()==true) cout<<"\tThe multiset 's1' is empty"<<endl;
 	else cout<<"\tThe multiset 's1' is not empty"<<endl;
  	cout<<"\tThe maximum capacity of the multiset 's1': "<<s1.max_size()<<endl;
 	...
 	cout<<"\tEnumerating the 'ulong' values in the multiset 's1' (in the reverse direction): ";
  	...
  	cout<<endl<<endl;
  	cout.flush();
  	for(unsigned int k=0;k<9;k++) { cout<<"\t#duplicated instances of the 'ulong' value "<<ls[k]<<": "<<s1.count(ls[k])<<endl; }
  	cout<<endl;
  	cout.flush();
    ...
    if(s0.empty()==true) cout<<"\tThe multiset 's0' is empty"<<endl;
 	...
 	cout<<"\t#values in the multiset 's1': "<<s1.size()<<endl;
    if(s1.empty()==true) cout<<"\tThe multiset 's1' is empty"<<endl<<endl;
 	else cout<<"\tThe multiset 's1' is not empty"<<endl<<endl;
 	cout.flush();
 	...
 	/* TASK #4 - validating the constructor, which creates a copy of an existing multiset of 'ulong' values. */
  	...
  	cout<<"ok"<<endl;
  	cout<<"\t#values in the multiset 's2': "<<s2.size()<<endl;
    if(s2.empty()==true) cout<<"\tThe multiset 's2' is empty"<<endl;
 	...
 	/* TASK #5 - validating the constructor, which creates a copy of an existing multiset of 'ulong' values. */
  	...
  	cout<<"ok"<<endl;
  	cout<<"\t#values in the multiset 's3': "<<s3.size()<<endl;
    if(s3.empty()==true) cout<<"\tThe multiset 's3' is empty"<<endl;
 	else cout<<"\tThe multiset 's3' is not empty"<<endl;
 	if(s2==s3) cout<<"\tThe 'ulong' values in the multisets 's2' and 's3' coincide (position by position)"<<endl;
 	else cout<<"\tThe 'ulong' values in the multisets 's2' and 's3' coincide (position by position)"<<endl;
 	if(s3.count(45)==0) cout<<"\tThe multiset 's3' does not contain the 'ulong' value 45"<<endl;
 	else cout<<"\tThe multiset 's3' contains the 'ulong' value 45"<<endl;
 	ulong n=s3.count((ulong)10);
 	if(n==0) cout<<"\tThe multiset 's3' does not contain the 'ulong' value 10"<<endl;
 	else cout<<"\tThe multiset 's3' contains "<<n<<" duplicates of the 'ulong' value 10"<<endl;
 	cout.flush();
 	auto it=s3.upper_bound(15);
 	cout<<"\tThe first 'ulong' value of the multiset 's3', which is considered to go after 15 (see the 'set::upper_bound()' member function), is: "<<(*it)<<endl;
 	auto it1=s3.lower_bound(29);
 	cout<<"\tThe first 'ulong' value of the multiset 's3', which is not considered to go before 29 (see the 'set::lower_bound()' member function), is: "<<(*it1)<<endl;
 	cout.flush();
 	auto it2=s3.equal_range(35);
 	cout<<"\tThe first 'ulong' value of the multiset 's3', which is not considered to go before 35 (see the 'set::equal_range()' member function), is: "<<(ulong)(*(it2.first))<<endl;
 	cout<<"\tThe first 'ulong' value of the multiset 's3', which is considered to go after 35 (see the 'set::equal_range()' member function), is: "<<(ulong)(*(it2.second))<<endl<<endl;
 	cout.flush();
 	
 	/* TASK #6 - validating the constructor, which creates a new multiset with a custom function. */
 	cout<<"\tCreating a new multiset 's4' by using a custom sorting function ... ";
 	multiset<ulong,bool(*)(const ulong&,const ulong&)> s4(compare_ulong_values);
 	cout<<"ok"<<endl;
 	cout<<"\t#values in the multiset 's4': "<<s4.size()<<endl;
    ...
    if(s4.empty()==true) cout<<"\tThe multiset 's4' is empty"<<endl;
 	...
 	cout<<"\tEnumerating the 'ulong' values in the multiset 's4' (in the forward direction): ";
  	for(auto it=s4.begin();it!=s4.cend();it++) cout<<(ulong)(*it)<<" ";
  	cout<<endl;
  	cout<<"\tEnumerating the 'ulong' values in the multiset 's4' (in the reverse direction): ";
  	for(auto it=s4.rbegin();it!=s4.rend();it++) cout<<(ulong)(*it)<<" ";
  	cout<<endl;
  	if(s4.find(45)==s4.end()) cout<<"\tThe 'ulong' value 45 does not belong to the multiset 's4'"<<endl;
  	else cout<<"\tThe 'ulong' value 45 belongs to the multiset 's4'"<<endl;
 	if(s4.find(10)==s4.end()) cout<<"\tThe 'ulong' value 10 does not belong to the multiset 's4'"<<endl;
 	else
 	{
 		cout<<"\tThe multiset 's4' contains "<<s4.count(10)<<" duplicates of the 'ulong' value 10"<<endl;
 		cout<<"\tRemoving all duplicates of the 'ulong' value '10' from the multiset 's4' ... ";
 		s4.erase(10);
 		cout<<"ok"<<endl;
 		cout<<"\t#values in the multiset 's4': "<<s4.size()<<endl;
 		if(s4.find(10)==s4.end()) cout<<"\tThe 'ulong' value 10 does not belong to the multiset 's4'"<<endl;
 		else cout<<"\tThe 'ulong' value 10 belongs to the multiset 's4'"<<endl;
 	}
 	...
 	/* TASK #7 - validating the constructor, which creates a new multiset with a custom (struct) function. */
  	cout<<"\tCreating a new multiset 's5' by using a custom (sorting) struct ... ";
